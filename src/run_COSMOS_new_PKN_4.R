## Run COSMOS with new network 
# Author: Diego Mañanes
# Description: run COSMOS with data from -- to check compatibility with new PKN
...
hs.PKN.COSMOS <- readRDS(file.path(outputPath, "COSMOS.PKN.hs.9606.rds"))
meta_network <- hs.PKN.COSMOS[[1]][[2]]
...
saveRDS(test_for, file.path(outputPath, "new_PKN_test_for_4.rds"))
saveRDS(test_result_for, file.path(outputPath, "new_PKN_test_result_for_4.rds"))
saveRDS(
  formatted_res_for, file.path(outputPath, "new_PKN_formatted_res_for_4.rds")
)
